  function wireUi(){
    ...
    // Collapsible sidebar sections — remember open/closed state per section
    (() => {
      const sections = $$('details.section');
      let saved: AnyRecord = {};
      try { saved = JSON.parse(localStorage.getItem('graph-editor-sections') || '{}') || {}; } catch {}
      sections.forEach((det, i) => {
        const keyEl = det.querySelector('[data-i18n]');
        const key = (keyEl && keyEl.dataset.i18n) || `section-${i}`;
        if(typeof saved[key] === 'boolean') det.open = saved[key];
        det.addEventListener('toggle', () => {
          saved[key] = det.open;
          try { localStorage.setItem('graph-editor-sections', JSON.stringify(saved)); } catch {}
        });
      });
    })();
