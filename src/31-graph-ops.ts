  ...
  function addEdge(from,to){
    ...
    const s = state.settings;
    const e: GraphEdge = { id:'e' + state.nextEdge++, from, to, label: String(s.edgeLabel || '').slice(0,80), directed: s.directed, type:'' };
  ...
  function setMatrixCell(from, to, raw){
    ...
    if(values.length === 0){
      ...
      for(const weight of values){
        const e: GraphEdge = { id:'e' + state.nextEdge++, from, to, weight:weight.slice(0,50), label:'', directed:s.directed, type:'' };
