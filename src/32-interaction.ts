  ...
  svg.addEventListener('pointerdown', ev => {
    ...
    if(ev.button !== 0) return;
    if((ev.target as Element)?.closest && (ev.target as Element).closest('.node,.edge')) return;
    if(polygonToolActive()){
      pendingEdgeFrom = null;
      handlePolygonPointerDown(ev);
      return;
  ...
  function finishPointer(ev, cancel=false){
    ...
    if(edgeDraft){
      ...
      const target = elAtPoint?.closest && elAtPoint.closest('.node');
      const to = (target as HTMLElement | null)?.dataset.id;
  ...
  function editEdgeQuick(id){ const e=edgeById(id); if(!e) return; const val=prompt(I18N.t('edge_weight_prompt'), String(e.weight ?? '')); if(val !== null){ const t = val.trim().slice(0,50); if(t === '') delete e.weight; else e.weight = t; pushHistory('edit edge'); queueRender(true); } }
